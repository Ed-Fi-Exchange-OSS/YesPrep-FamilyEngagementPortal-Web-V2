angular.module('app')
    .config(['$stateProvider', function($stateProvider) {
        $stateProvider.state('app.reportMessageParent', {
                url: '/reportmessageparent',
                requireADLogin: true,
                views: {
                    'content@': { component:'reportMessageParent' }
                }
            });
    }])
    .component('reportMessageParent', {
        bindings: {
            model: "<",
            tooltips: "<"
        }, // One way data binding.
        templateUrl: 'clientapp/modules/report/reportMessage/reportMessageParent.view.html',
        controllerAs:'ctrl',
        controller: ['api', '$translate', function (api) {
            ...
            ctrl.exportAllParentCSV = function ($event) {
                ...
                api.logAccess.getParentMessageByDistrict().then(function (r) {
                    ctrl.downloadFile(r, "ParentListMessages");
                });
            };

            ctrl.exportParentCSV = function ($event, campus, educationOrganizationId) {
                $event.preventDefault(); 
                api.logAccess.getParentMessageByCampus(educationOrganizationId).then(function (r) {
