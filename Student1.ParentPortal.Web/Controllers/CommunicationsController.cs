...
namespace Student1.ParentPortal.Web.Controllers
{
    ...
    public class CommunicationsController : ApiController
    {
        ...
        public async Task<IHttpActionResult> RecipientRecentMessages(AllRecipientsRequestModel request)
        {
            ...
            else if (role.Equals("Admin", System.StringComparison.InvariantCultureIgnoreCase) || role.Equals("CampusLeader", System.StringComparison.InvariantCultureIgnoreCase))
            {
                return Ok(await _communicationsService.GetAllCampuLeaderRecipients(request.StudentUsi, person.PersonUniqueId, person.PersonTypeId, request.RowsToSkip, request.RowsToRetrieve));
            }
            else
            {
        ...
        [Route("families/searchstudents")]
        [HttpPost]
        public async Task<IHttpActionResult> GetStudentByTermAndGradeLevel(ParentStudentTermFilterModel parentStudentTermFilter)
        {
            var campusLeader = SecurityPrincipal.Current;
            var model = await _communicationsService.GetStudentByTermAndGradeLevel(campusLeader.PersonUniqueId, parentStudentTermFilter.SearchTerm, parentStudentTermFilter.GradeLevels, parentStudentTermFilter.SchoolId);
        ...
        [Route("families/searchstudentsprograms")]
        [HttpPost]
        public async Task<IHttpActionResult> GetStudentsByTermGradeLevelAndPrograms(ParentStudentTermProgramFilterModel parentStudentTermFilter)
        {
            var campusLeader = SecurityPrincipal.Current;
            var model = await _communicationsService.GetStudentByTermGradeLevelAndProgram(campusLeader.PersonUniqueId, parentStudentTermFilter.SearchTerm, parentStudentTermFilter.GradeLevels, parentStudentTermFilter.SchoolId, parentStudentTermFilter.Programs);
