...
namespace Student1.ParentPortal.Data.Models.EdFi31
{
    public class NotificationsRepository : INotificationsRepository
    {
        ...
        {
            return await _edFiDb.NotificationsTokens.Join(
                    _edFiDb.Parents,
                    notificationToken=> notificationToken.PersonUniqueId,
                    parent=> parent.ParentUniqueId,
                    (notificationToken, parent)=>new { NotificationToken = notificationToken, Parent = parent }
                )
                .Where(n => n.Parent.ParentElectronicMails.FirstOrDefault().ElectronicMailAddress==email)
                .Select(n => n.NotificationToken.Token).ToArrayAsync();
        }
        ...
        public async Task SaveNotificationIdentifier(NotificationsIdentifierModel model)
        {
            ...
            if(fieldExist != null ) 
            {
                if(fieldExist.Token != model.Token)
                {
