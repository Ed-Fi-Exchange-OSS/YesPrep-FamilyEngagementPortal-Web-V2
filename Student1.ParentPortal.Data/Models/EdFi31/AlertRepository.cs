    ...
    public class AlertRepository : IAlertRepository
    {
        ...
        public async Task SetParentUnreadAlert()
        {
            var parents = await (from p in _edFiDb.Parents
                          where !_edFiDb.ParentAlerts.Any(pa => pa.ParentUniqueId == p.ParentUniqueId) && !_edFiDb.ParentUnreadAlertCreateLogs.Any(pa => pa.ParentUniqueId == p.ParentUniqueId)
                                 select p.ParentUniqueId).ToListAsync();

            List<ParentUnreadAlertCreateLog> parentsList = new List<ParentUnreadAlertCreateLog>();
            foreach (var p in parents)
            {
                parentsList.Add(new ParentUnreadAlertCreateLog { ParentUniqueId = p });
            }
            if (parentsList.Count > 0)
            {
                _edFiDb.ParentUnreadAlertCreateLogs.AddRange(parentsList);
        ...
        private ParentAlert createParentAlert(string uniqueId)
        {
            ...
            newParentAlert.AlertsEnabled = true;
            newParentAlert.ParentUniqueId = uniqueId;

            return newParentAlert;
        }

        public async Task<List<StudentAlertModel>> studentsOverThresholdAbsence(decimal excusedThreshold, decimal unexcusedThreshold, decimal tardyThreshold, string excusedDescriptor, string unexcusedDescriptor, string tardyDescriptor, DateTime today)
        {
            var studentsOverThreshold = await (from s in _edFiDb.Students
                                               ...
                                               join gr in _edFiDb.Grades
                                                        .Include(x => x.GradingPeriod)
                                                        on s.StudentUsi equals gr.StudentUsi
                                               where pa.AlertsEnabled
                                                   && sy.CurrentSchoolYear // Only for current School Year
                                                   && gr.GradingPeriod.BeginDate <= today //For the current grading period
                                                   && gr.GradingPeriod.EndDate >= today
                                               group new { s, ssae, pa, pro, spa } by  new { s.StudentUsi, pa.ParentUniqueId } into g
                                               ...
                                               select new
                                               {
                                                   ...
                                                   Data = g.GroupBy(x => x.pa.ParentUniqueId).Select(data => new StudentParentsAlertsProfilesModel
                                                   {
                                                       ...
                                                       ParentProfile = g.Select(x => x.pro).Where(x => x.ParentUniqueId == data.FirstOrDefault().spa.Parent.ParentUniqueId).FirstOrDefault()
                                                   }),
        ...
        public async Task<List<StudentAlertModel>> studentsOverThresholdAdaAbsence(decimal unexcusedThreshold, string absentDescriptor, DateTime today)
        {
            var studentsOverThreshold = await (from s in _edFiDb.Students
                                               join spa in _edFiDb.StudentParentAssociations
                                                           .Include(x => x.Parent.ParentElectronicMails)
        ...
        public async Task<List<StudentAlertModel>> studentsOverThresholdBehavior(decimal behaviorThreshold, string disciplineIncidentDescriptor, DateTime today)
        {
            var studentsOverThreshold = await (from s in _edFiDb.Students
                                               join spa in _edFiDb.StudentParentAssociations
                                                           .Include(x => x.Parent.ParentElectronicMails)
                                               ...
                                               where pa.AlertsEnabled
                                                ...
                                                && gra.GradingPeriod.BeginDate <= today //For the current grading period
                                                && gra.GradingPeriod.EndDate <= today
                                               group new { s, pa, pro, spa } by new { s.StudentUsi, pa.ParentUniqueId } into g
        ...
        public async Task<List<StudentAlertModel>> studentsOverThresholdAssignment(decimal assignmentThreshold, string[] gradeBookMissingAssignmentTypeDescriptors, string missingAssignmentLetterGrade, DateTime today)
        {
            ...
            var studentsOverThreshold = await (from s in _edFiDb.Students
                                               ...
                                               join ge in _edFiDb.GradebookEntries on
                                                        new { sge.DateAssigned, sge.GradebookEntryTitle, sge.LocalCourseCode, sge.SchoolId, sge.SchoolYear, sge.SectionIdentifier, sge.SessionName } equals
                                                        new { ge.DateAssigned, ge.GradebookEntryTitle, ge.LocalCourseCode, ge.SchoolId, ge.SchoolYear, ge.SectionIdentifier, ge.SessionName }

                                               join gp in _edFiDb.GradingPeriods on
                                                       new { ge.SchoolId, ge.SchoolYear, PeriodSequence = ge.PeriodSequence.Value } equals
                                               ...
                                               && gp.BeginDate <= today //For the current grading period
                                               && gp.EndDate >= today
                                               && sy.CurrentSchoolYear
                                               group new { s, pa, pro, spa } by new { s.StudentUsi, pa.ParentUniqueId } into g
        ...
        public async Task<List<StudentAlertModel>> studentsOverThresholdCourse(decimal courseThreshold, string gradeTypeGradingPeriodDescriptor, DateTime today)
        {
                ...
                var studentsOverThreshold = await (from s in _edFiDb.Students
                                                   ...
                                                   where pa.AlertsEnabled &&
                                                           ...
                                                           && gra.GradingPeriod.BeginDate <= today //For current grading period
                                                           && gra.GradingPeriod.EndDate >= today
                                                   orderby gra.GradingPeriod.GradingPeriodDescriptor.Descriptor.CodeValue ascending
                                                   group new { s, co, gra, pa, pro, spa } by s.StudentUsi into g
                ...
                var newdata = studentsOverThreshold
                    .Select(g => new {
                        ...
                        Data = g.GroupBy(x => x.pa.ParentUniqueId).Select(data => new StudentParentsAlertsProfilesModel
                        {
                            StudentParentAssociation = data.FirstOrDefault().spa,
                            ParentAlert = g.Select(x => x.pa).Where(x => x.ParentUniqueId == data.FirstOrDefault().spa.Parent.ParentUniqueId).FirstOrDefault(),
                            ParentProfile = g.Select(x => x.pro).OfType<ParentProfile>().Where(x => x.ParentUniqueId == data.FirstOrDefault().spa.Parent.ParentUniqueId).FirstOrDefault()
                        }),
                        courseCount = g.Count(),
        ...
        public async Task<List<UnreadMessageAlertModel>> ParentsAndStaffWithUnreadMessages()
        {
            ...
            var parents = await (from p in _edFiDb.Parents
                                 ...
                                 from palerts in _edFiDb.ParentAlerts
                                                .Where(x => x.ParentUniqueId == p.ParentUniqueId)
                                                .DefaultIfEmpty()
                                 from pro in _edFiDb.ParentProfiles
                                               .Where(x => x.ParentUniqueId == p.ParentUniqueId)
                                               .DefaultIfEmpty()
                                 ...
                                 from pt in _edFiDb.ParentTelephones.Where(x =>
                                 ...
                                 where _edFiDb.ChatLogs.Any(cl => cl.RecipientTypeId == ChatLogPersonTypeEnum.Parent.Value && cl.RecipientUniqueId == p.ParentUniqueId && !cl.RecipientHasRead) &&
                                     palerts != null && palerts.AlertsEnabled
                                 select new ParentProfileAlertModel { Parent = p, ParentProfile = pro, ParentAlert = palerts, ParentTelephone = pt }).ToListAsync();
            ...
            var returnParents = parents.Select(x => new UnreadMessageAlertModel
            {
                ...
                Telephone = x.ParentTelephone != null ? x.ParentTelephone?.TelephoneNumber : getParentTelephone(x.ParentProfile),
                SMSDomain = getParentSMSDomain(x.ParentProfile),
                PreferredMethodOfContactTypeId = x.ParentProfile != null ? x.ParentProfile?.PreferredMethodOfContactTypeId : x.ParentTelephone != null ? MethodOfContactTypeEnum.SMS.Value : MethodOfContactTypeEnum.Email.Value,
                AlertTypeIds = x.ParentAlert.AlertTypes.Select(pa => pa.AlertTypeId).ToList(),
                PersonUniqueId = x.Parent.ParentUniqueId,
                PersonType = "Parent",
                LanguageCode = x.ParentProfile != null ? x.ParentProfile.LanguageCode : ""
            }).ToList();
            ...
            var returnstaffs = staffs.Select(x => new UnreadMessageAlertModel
            {
                ...
                PreferredMethodOfContactTypeId = x.Profile != null ? x.Profile.PreferredMethodOfContactTypeId : MethodOfContactTypeEnum.Email.Value,
        ...
        private class ParentProfileAlertModel
        {
            ...
            public ParentAlert ParentAlert { get; set; }
            public ParentProfile ParentProfile { get; set; }
            public ParentTelephone ParentTelephone { get; set; }
        }
    }
}
